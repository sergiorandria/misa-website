    ...
    <header className="sticky top-0 z-50 bg-white border-b border-[var(--color-misa-line)]">
      <div className="max-w-[1160px] mx-auto px-6 h-[64px] flex items-center justify-between gap-6">
        <Link to="/" className="flex items-center gap-3 shrink-0">
          <img src="/logo-mit.png" alt="MIT" className="h-9 w-auto object-contain" />
        ...
        <nav className="hidden lg:flex items-center gap-6">
          ...
          <div className="relative" onMouseEnter={() => setDrop(true)} onMouseLeave={() => setDrop(false)}>
            <button className={`${linkBase} ${linkIdle} flex items-center gap-1`}>
              FORMATION <ChevronDown size={14} className={`${drop ? "rotate-180" : ""} transition`} />
            </button>
            {drop && (
              <div className="absolute left-1/2 -translate-x-1/2 top-full mt-3 w-[420px] bg-white border border-[var(--color-misa-line)] p-2 flex gap-2 shadow-sm">
                {[
                  ...
                  <Link key={i.to} to={i.to} className="flex-1 border border-[var(--color-misa-line)] p-3 hover:border-[var(--color-misa-ink)] transition">
