  ...
  return (
    <div className="bg-white page-fade-in">
      <div className="max-w-[1160px] mx-auto px-6 py-8">
        <div className="text-xs tracking-widest text-[var(--color-misa-red)] font-semibold">FORMATION — MASTER</div>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-[var(--color-misa-ink)]">Master</h1>
      ...
      <div ref={pageRef} className="reveal max-w-[1160px] mx-auto px-6 pb-12">
        <div className="flex gap-2">
          {[
            ...
            <button
              key={k}
              onClick={() => { setParcours(k); setLevel("M1"); }}
              className={`text-sm font-semibold px-4 py-2 border transition-all duration-200 cursor-pointer ${
                parcours === k
        ...
        <div key={parcours} className="mt-4 border border-[var(--color-misa-line)] rounded-sm shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden animate-fadeIn">
          <div className="flex flex-wrap items-center justify-between gap-4 p-5 border-b border-[var(--color-misa-line)] bg-[var(--color-misa-paper)]">
            <div>
              <h2 className="text-lg font-bold text-[var(--color-misa-ink)]">{data.title}</h2>
            ...
            <div className="flex gap-2">
              {["M1","M2"].map(l => (
                ...
                >
                  Master {l.slice(1)} <span className="opacity-60">{l === "M1" ? "S7·S8" : "S9·S10"}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-[1.4fr_0.6fr]">
            <div className="p-5">
              <div className="text-xs tracking-widest text-neutral-500 font-semibold mb-3">UE — MASTER {level.slice(1)}</div>
              <div key={`${parcours}-${level}`} className="grid sm:grid-cols-2 gap-3 animate-fadeIn">
                {groups.map((g, idx) => (
            ...
            <div className="border-t lg:border-t-0 lg:border-l border-[var(--color-misa-line)] bg-[var(--color-misa-paper)] p-5 space-y-4">
              <div>
                ...
                <div className="mt-1 text-sm font-semibold text-[var(--color-misa-ink)]">{data.responsable}</div>
              </div>
